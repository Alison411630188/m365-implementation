...
const cases: Record<string, CaseDetail> = {
  ...
  case4: {
    ...
    recommendedTools: [
      "List：新進員工入職檢查清單與進度追蹤",
      "Planner：入職任務與里程碑管理",
      "SharePoint：入職資訊、政策、表單與培訓資料集中存放",
      "Teams：新進員工導引與溝通",
    ],
    operationFlow: [
      "HR 於 List 中建立『新進員工入職清單』，列出所有必要的入職步驟（如系統帳號開通、設備領取、部門導引、政策簽署等）",
      "HR 為每位新進員工建立一筆記錄，填寫入職日期、部門、職位、導師與完成期限",
      "HR 或導師於 Planner 中為新進員工建立『入職計畫』，依時間軸安排各項入職任務與培訓課程",
      "新進員工於 List 或 Planner 中查看入職清單與任務，逐項完成並更新進度",
      "導師與部門主管可於 List 與 Planner 中檢視新進員工的入職進度，及時提供協助與指導",
      "HR 可定期檢視 List 中的入職進度，確保所有新進員工都按時完成入職流程",
      "將新進員工所需的政策、表單、培訓資料與常見問題集中存放於 SharePoint，新進員工可隨時查閱",
      "入職完成後，HR 可根據 Lists 與 Planner 的記錄進行總結，持續改進入職流程",
    ],
    supplementaryNotes: [
      "List 的檢查清單功能可幫助追蹤每項入職步驟的完成狀態",
    ...
    useCase:
      "公司每月平均招聘 5-10 名新進員工。HR 於 List 中建立『新進員工入職清單』，列出 20 項標準入職步驟。新進員工入職時，HR 為其建立一筆記錄，並在 Planner 中建立『入職計畫』，安排為期 4 週的入職課程與任務。新進員工可在 List 與 Planner 中查看自己的入職進度，並在完成各項步驟時更新狀態。導師與部門主管可隨時檢視新進員工的入職進度，及時提供協助。HR 每月檢視 List 與 Planner 的數據，評估入職流程的效率與效果，並持續優化。",
    expectedBenefits: [
      "新進員工入職流程標準化，減少遺漏與重複",
  ...
  case5: {
    ...
    operationFlow: [
      "HR 於 List 中建立『申請單據』清單，設定欄位包括『申請類型』（假單、加班、採購等）、『申請人』、『申請日期』、『申請內容』、『簽核人』、『簽核狀態』與『備註』",
      "員工於 List 中提交申請單，填寫申請類型、內容與相關附件",
      "利用 Power Automate 建立簽核流程，當員工提交申請時，自動通知相應的簽核人員進行審核",
      "簽核人員於 List 或 Teams 中查看待簽核的申請單，並進行核准或駁回",
      "申請單的簽核狀態自動更新，員工可於 List 或 Teams 中查詢申請進度",
      "申請核准後，Power Automate 可自動執行後續流程（如發送確認郵件、更新人資系統等）",
      "員工可於任何時間查詢自己的申請歷史與核准狀況，便於後續參考",
      "HR 可定期檢視 List 中的申請數據，進行統計分析與流程優化",
    ],
    supplementaryNotes: [
      "List 的欄位設計應清楚區分不同類型的申請，便於篩選與管理",
    ...
    useCase:
      "員工需要申請年假。員工於 List 中提交假單申請，填寫假期類型、起訖日期與原因。系統自動通知部門主管進行審核。主管於 List 中查看申請單，並進行核准或駁回。申請狀態自動更新，員工於 Teams 中收到通知。員工可隨時於 List 中查詢申請進度。核准後，系統自動發送確認郵件，並更新人資系統的假期記錄。",
    expectedBenefits: [
  ...
  case6: {
    ...
    recommendedTools: [
      ...
      "HR 或管理層於 List 中建立『現場事件通報』清單，設定欄位包括『事件類型』、『通報人』、『通報時間』、『事件位置』、『事件描述』、『優先級』、『處理人』、『處理狀態』與『處理結果』",
      "現場人員發現事件時，於 List 中立即提交事件通報，填寫事件類型、位置、描述與優先級，並上傳相關照片或文件",
      "利用 Power Automate 設定自動通知，當事件被提交時，自動通知值班主管與相應的處理人員進行處理",
      "值班主管於 List 或 Teams 中查看待處理的事件，並指派給對應的處理人員",
      "處理人員於 List 中更新事件處理進度（如『已接收』、『處理中』、『已解決』等），並上傳處理結果與照片",
      "現場人員與值班主管可實時查看事件的處理進度，及時了解狀況",
      "事件解決後，處理人員填寫『處理結果』與『完成時間』，事件記錄自動歸檔",
      "管理層可定期檢視 List 中的事件數據，利用 Power BI 進行統計分析，識別常見事件類型與改進機會", ],
    supplementaryNotes: [
      "List 的優先級欄位應清楚區分事件緊急度，便於快速處理",
    ...
    useCase:
      "工廠現場發生安全事件。現場人員於 List 中立即提交事件通報，填寫事件類型（安全事件）、位置、描述與優先級（高），並上傳現場照片。系統自動通知值班主管與安全人員。值班主管於 Teams 中收到通知，立即查看事件詳情，並指派給安全人員進行處理。安全人員到達現場後，於 List 中更新處理進度，上傳處理措施與照片。現場人員與值班主管可實時查看處理進度。事件解決後，安全人員填寫『處理結果』，事件記錄自動歸檔。管理層定期檢視事件數據，識別常見問題並進行改進。",
    expectedBenefits: [
      "事件通報即時，減少處理延誤",
  ...
  case7: {
    ...
    operationFlow: [
      "HR 建立『員工滿意度調查』問卷，包括工作環境、薪酬福利、職涯發展、管理風格等多個維度",
      "HR 定期（如每季或每半年）發送問卷給全體員工或特定部門，邀請員工填寫回饋",
      "員工匿名或實名填寫問卷，提供意見與建議",
      "系統自動收集回饋數據，並將結果存放於 List 中進行進一步管理與分類",
      "HR 於 List 中對回饋進行分類、標籤與優先級設定，便於後續分析與跟進",
      "HR 利用 Power BI 對回饋數據進行分析，生成滿意度報告、趨勢分析與關鍵問題識別",
      "HR 與部門主管於 Teams 中分享回饋結果與分析報告，討論改進措施",
      "根據回饋結果，HR 與部門主管制定改進計畫，並在 List 中追蹤改進措施的執行進度",
      "定期檢視改進措施的成效，並在下一次調查中評估滿意度是否有所提升",
    ],
    supplementaryNotes: [
      "Forms 的匿名功能可鼓勵員工提供坦誠的意見，提高回饋品質",
      "List 的分類與標籤功能可幫助快速識別常見問題與改進機會",
    ...
    useCase:
      "HR 建立『員工滿意度調查』問卷，包括 20 個問題涵蓋工作環境、薪酬、發展機會等維度。HR 邀請全體員工填寫問卷，回收率達 80%。系統自動收集數據，HR 將結果匯入 List 進行分類。HR 利用 Power BI 生成分析報告，發現『職涯發展機會』與『工作與生活平衡』是員工最關注的兩個問題。HR 與各部門主管於 Teams 中討論結果，制定改進計畫（如提供更多培訓機會、調整工作時程等）。三個月後進行下一次調查，評估改進措施的成效。",
    expectedBenefits: [
      "回饋蒐集標準化，確保覆蓋所有重要維度",
      "回饋數據集中管理，便於分析與追蹤",
      "快速識別員工關鍵問題與改進機會",
